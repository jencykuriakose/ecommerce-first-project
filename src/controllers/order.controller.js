...
const VerifyPayment = async (req, res) => {
	...
	if (VerifyResult) {
		...
		const changePaymentResult = await ordermodel.changePaymentStatus(req.body["order[receipt]"], paymentDetails);
...
const GetWallet = async (req, res) => {
	try {
		...
		const transactions = await transactionDatabase.find({ user: user._id });
		console.log(transactions);
		const walletAmount = await ordermodel.getwallet(req.session.user._id);
		return res.render("user/wallet", { transactions,user});
	} catch (error) {
		handleError(res, error);
	}
};
...
const ApplyWallet = async (req, res) => {
	try {
		...
		await transactionDatabase.insertMany([
			{
				user: userId,
				amount: walletAmount,
				type: "Debited",
